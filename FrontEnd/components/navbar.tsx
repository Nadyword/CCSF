  ...
  return (
    <header className="fixed top-0 z-50 w-full bg-background/95 backdrop-blur-md shadow-lg shadow-black/5 border-b transition-all duration-300">
      <nav className="relative mx-auto flex h-16 max-w-7xl items-center px-4 sm:px-6 lg:px-8">
        ...
        {/* Logo + Nav centrados en desktop */}
        <div className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 md:flex">
          <Link href="/" className="group flex items-center mr-3">
            <div className="relative">
              <div className="absolute inset-0 rounded-full bg-[var(--brand-primary)]/10 blur-lg group-hover:scale-125 transition-all duration-300" />
              <Image
                src={infoCC.logo}
                ...
                width={56}
                height={56}
                className="relative h-14 w-14 object-contain transition-all duration-300"
              />
            </div>
          </Link>
          {navLinks.map((link) => (
        ...
        {/* Derecha: dropdown autenticado + hamburger móvil */}
