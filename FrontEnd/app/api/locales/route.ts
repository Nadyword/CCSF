  ...
  try {
    ...
    const result = await Promise.all(
      items.map(async name => {
        const s = await stat(path.join(LOCALES_DIR, name))
        return { name, url: `/Locales/${name}`, size: s.size, modified: s.mtime.toISOString() }
      })
    )
...
export async function POST(req: NextRequest) {
  ...
  await writeFile(path.join(LOCALES_DIR, name), Buffer.from(bytes))

  return NextResponse.json({ name, url: `/Locales/${name}` }, { status: 201 })
}
